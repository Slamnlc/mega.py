...
class Mega:
    ...
    def _api_request(self, data):
        ...
        try:
            if isinstance(json_resp, list):
                int_resp = (
                    json_resp[0] if isinstance(json_resp[0], int) else None
                )
        ...
        for file in list(files.items()):
            ...
            try:
                if parent_dir_name:
                    ...
                    filename and file[1]["a"] and file[1]["a"]["n"] == filename
                ):
                    if (
                        exclude_deleted
                        and self._trash_folder_node_id == file[1]["p"]
    ...
    def _export_file(self, node):
        ...
        self._api_request(
            [{"a": "l", "n": node_data["h"], "i": self.request_id}]
        )
    ...
    def upload(
        ...
        dest=None,
        dest_filename=None,
        progress_func: Callable[[int, int], None] = None,
    ):
        ...
        with open(filename, "rb") as input_file:
            ...
            if file_size > 0:
                for chunk_start, chunk_size in get_chunks(file_size):
                    ...
                    if progress_func:
                        progress_func(upload_progress, file_size)
            else:
                output_file = requests.post(
    ...
    def move(self, file_id, target):
        ...
        return self._api_request(
            {"a": "m", "n": file_id, "t": target_node_id, "i": self.request_id}
        )
    ...
    def _edit_contact(self, email, add):
        ...
        if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
            ...
            return self._api_request(
                {"a": "ur", "u": email, "l": l, "i": self.request_id}
            )
